...
function  getDeviceList(client, village_id, type, callback){

    if (type==1){
            //client为一个mysql连接对象
        client.query('select device_id,device_pos,device_name,V_level,pos_x,pos_y from tb_device where V_level=110 and village_id='+village_id ,function(err,results,fields){
           if( err ){
                console.log( "error:" + err.message);
                return err;
            }
        ...
        //client为一个mysql连接对象
        client.query('select device_id,device_pos,device_name,V_level,pos_x,pos_y from tb_device where V_level=10 and village_id='+village_id ,function(err,results,fields){
           if( err ){
                console.log( "error:" + err.message);
                return err;
            }
...
function addDevice(client,village_id ,device_name,device_pos_x,device_pos_y,v_level, callback) {
    client.query('insert into tb_device (device_name,village_id,pos_x,pos_y,V_level) values (?,?,?,?,?)', 
        [device_name, village_id, device_pos_x, device_pos_y,v_level],function(err,results){
        if( err ){
            console.log( "error:" + err.message);
            return err;
        } 
...
function  updateModule(client, module_id, module_name,module_type,producer,running_date,
    ...
    if (bianbi){
        sql = sql + ' bianbi=' + bianbi + ','
    }
    if (dingzhidanhao){
        sql = sql + ' dingzhidanhao=' + dingzhidanhao + ','
    }
...
function  insertModule(client, device_id,module_name,module_type,producer,running_date,
       verify_date,version,verify_code,net_address,bianbi,dingzhidanhao, callback){
    //client为一个mysql连接对象
    sql = 'insert into tb_module (module_name,module_type,producer,running_date,verify_date,version,verify_code,net_address,bianbi,dingzhidanhao,device_id)'
        + 'values ("' + module_name + '","' + module_type + '","' + producer + '","' + running_date
        + '","' + verify_date + '",' + version + ',' + verify_code 
        + ',"' + net_address + '","'  + bianbi + '","'  + dingzhidanhao + '",'
        + device_id + ')';
